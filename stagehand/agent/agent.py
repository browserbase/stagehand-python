...
class Agent:
    ...
    def __init__(self, stagehand_client, **kwargs):
        ...
        # If no model specified in agent config, use the model from stagehand client
        if not self.config.model:
            self.config.model = self.stagehand.model_name
            
        self.logger = self.stagehand.logger
        # API mode removed - always use local mode
        if False:  # Disabled API mode
